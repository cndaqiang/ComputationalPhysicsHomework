...
module IsingFuns
    ...
    subroutine IsingAnalyze(E,M,T,sampleStart,sampleEnd,sampleNum,aveM, aveE, varM, varE,CorrE,tao)
        ...
        CorrE(i)=( (sampleEnd-tao(i)-sampleStart+1)*SUM(E(sampleStart:sampleEnd-tao(i))*E(sampleStart+tao(i):sampleEnd)) -&
                       SUM(E(sampleStart:sampleEnd-tao(i)))**2    )/ &
                     ( (sampleEnd-tao(i)-sampleStart+1)*SUM(E(sampleStart:sampleEnd-tao(i))*E(sampleStart:sampleEnd-tao(i))) -&
                     SUM(E(sampleStart:sampleEnd-tao(i)))**2    )
        ENDDO
