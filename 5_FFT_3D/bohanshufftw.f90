...
program bohanshufftw
    ...
    implicit none
    include 'fftw3.f03'
    INTEGER,PARAMETER :: N=4096 !N=600
    INTEGER :: i,j
    !波函数f(x)=c1*exp(i*f1*x)+c2*exp(i*f2*x)
    REAL,PARAMETER :: pi=3.141592653
    REAL(C_DOUBLE)::x(N,N),y(N,N),kx(N,N),ky(N,N),dx,dy,p1x,p1y,c1,p2x,p2y,c2
    ...
    DO j = 1,N
        Do i =1,N
        x(i,j)=(i-1.0)*dx
        kx(i,j)=2*pi*(i-1.0)/N/dx
        y(i,j)=(j-1.0)*dy
        ky(i,j)=2*pi*(j-1.0)/N/dy
        realf=c1*cos((p1x*x(i,j)+p1y*y(i,j)))+c2*cos((p2x*x(i,j)+p2y*y(i,j)))
        imagef=c1*sin((p1x*x(i,j)+p1y*y(i,j)))+c2*sin((p2x*x(i,j)+p2y*y(i,j)))
...
!-----------------------    
    ...
    DO j = 1,N
        Do i =1,N 
        if(x(i,j)<=1 .and. y(i,j)<1) then
            write(12,*) x(i,j),y(i,j),abs(fxy(i,j))
            write(14,*) x(i,j),y(i,j),abs(ffxy(i,j))
        endif
        if(kx(i,j)<=50 .and. ky(i,j)<=50) write(13,*) kx(i,j),ky(i,j),abs(fk(i,j))
